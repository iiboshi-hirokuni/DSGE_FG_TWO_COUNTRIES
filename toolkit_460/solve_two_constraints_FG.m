...
% constraint1_      = ['(r_h_difference     < istar) | (r_not_lag1_h_difference  < istar)',...
%                     ' | (r_not_lag2_h_difference  < istar)']; % effective lower bound on the nominal interest rates
% constraint_relax1_ =['(r_not_h_difference > istar) & (r_not_lag1_h_difference  > istar)', ....
%                     ' & (r_not_lag2_h_difference  > istar)' ] ;
% 
% constraint2_      = [ '(r_f_difference     < istar) | (r_not_lag1_f_difference  < istar)',...
%                      '| (r_not_lag2_f_difference  < istar) '] ; % effective lower bound on the nominal interest rates
% constraint_relax2_ = [ '(r_not_f_difference > istar) & (r_not_lag1_f_difference  > istar)',...
...
%%
         newviolvecbool1_  =  eval( constraint1_ ); 
         relaxconstraint1_ =  eval( constraint_relax1_ );
...
%%
         newviolvecbool2_ = eval(  [ (constraint2_) ] );
         relaxconstraint2_ = eval( [ (constraint_relax2_) ]  );         
