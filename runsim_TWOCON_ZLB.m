...
addpath('c:\dynare\4.6.3\matlab\occbin')
addpath('.\toolkit_460')
...
constraint1      = '(r_h     < istar) '; % effective lower bound on the nominal interest rates
constraint_relax1 ='(r_not_h > istar)';

constraint2      = '(r_f     < istar) '; % effective lower bound on the nominal interest rates
constraint_relax2 ='(r_not_f > istar)';
...
switch shock
 case 'GL_shock'
 %  shockssequence = [ zeros(1,1); -0.04*ones(1,1);    zeros(nperiods1,1) ];         % scale factor for simulations
...
%% Modify to plot IRFs 
 ...
 line1_zlb=100*[pi_h_piecewise,pi_f_piecewise,x_h_piecewise,x_f_piecewise,r_h_piecewise-istar,r_f_piecewise-istar,...
...
makechart(titlelist,legendlist,figlabel,ylabels,line1_zlb,line2_c);

 save ./cal_results/IRF_ZLB line1_zlb;

%%  Calculation Walfare 


% [ loss_zlb, loss_zlb_h, loss_zlb_f ] = func_cal_walfare(P,pi_h_piecewise,pi_f_piecewise,x_h_piecewise,x_f_piecewise,r_h_piecewise-istar,r_f_piecewise-istar );
[ loss_zlb, loss_zlb_h, loss_zlb_f ] = func_cal_walfare(P,pi_h_piecewise,pi_f_piecewise,x_h_piecewise,x_f_piecewise,r_h_piecewise,r_f_piecewise );

save('./cal_results/loss_zlb.mat', 'loss_zlb','loss_zlb_h', 'loss_zlb_f');

figure('Name','Loss Function of ZLB')
plot(loss_zlb, 'LineWidth',2) 
title('Loss of Two Countries for ZLB', 'FontSize',14)
