...
constraint1      = ['(r_h_difference     < istar) | (r_not_lag1_h_difference  < istar)',...
                    ...
                    ' | (r_not_lag4_h_difference  < istar) | (r_not_lag5_h_difference  < istar)',...
                     ' | (r_not_lag6_h_difference  < istar) | (r_not_lag7_h_difference  < istar)',...
                    ' | (r_not_lag8_h_difference  < istar) | (r_not_lag9_h_difference  < istar)',...
                    ' | (r_not_lag10_h_difference  < istar) | (r_not_lag11_h_difference  < istar)',...
                    ' | (r_not_lag12_h_difference  < istar)'  ]; % effective lower bound on the nominal interest rates
...
constraint_relax1 =['(r_not_h_difference > istar) & (r_not_lag1_h_difference  > istar)', ....
                    ...
                    ' & (r_not_lag8_h_difference  > istar) & (r_not_lag9_h_difference  > istar)',...
                    ' & (r_not_lag10_h_difference  > istar) & (r_not_lag11_h_difference  > istar)',...
                    ' & (r_not_lag12_h_difference  > istar)' ];
...
constraint2      = [ '(r_f_difference     < istar) | (r_not_lag1_f_difference  < istar)',...
                    ...
                    ' | (r_not_lag6_f_difference  < istar) | (r_not_lag7_f_difference  < istar)',...
                    ' | (r_not_lag8_f_difference  < istar) | (r_not_lag9_f_difference  < istar)',...
                    ' | (r_not_lag10_f_difference  < istar) | (r_not_lag11_f_difference  < istar)',...
                    ' | (r_not_lag12_f_difference  < istar)'  ]; % effective lower bound on the nominal interest rates
...
constraint_relax2 = [ '(r_not_f_difference > istar) & (r_not_lag1_f_difference  > istar)',...
                    ...
                    ' & (r_not_lag8_f_difference  > istar) & (r_not_lag9_f_difference  > istar)',...
                    ' & (r_not_lag10_f_difference  > istar) & (r_not_lag11_f_difference  > istar)',...
                    ' & (r_not_lag12_f_difference  > istar)'  ];
...
[loss_FG_12, loss_FG_12_h, loss_FG_12_f ] = func_cal_walfare(P,pi_h_piecewise,pi_f_piecewise,x_h_piecewise,x_f_piecewise,r_h_piecewise,r_f_piecewise );

save('./cal_results/loss_FG_12.mat', 'loss_FG_12', 'loss_FG_12_h','loss_FG_12_f');
% load('loss_zlb.mat');
% 
% 
% ti = linspace(0,12,13);
...
%%
 save ./cal_results/IRF_FG_12 line1_fg;
 ...
 figtitle = '';
 figlabel= '';
 legendlist = cellstr(char('FG  12Q Extra','ZLB'));
