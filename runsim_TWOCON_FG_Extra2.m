...
constraint1      = ['(r_h_difference     < istar) | (r_not_lag1_h_difference  < istar)',...
                    ' | (r_not_lag2_h_difference  < istar)']; % effective lower bound on the nominal interest rates
constraint_relax1 =['(r_not_h_difference > istar) & (r_not_lag1_h_difference  > istar)', ....
                    ' & (r_not_lag2_h_difference  > istar)' ] ;

constraint2      = [ '(r_f_difference     < istar) | (r_not_lag1_f_difference  < istar)',...
                     '| (r_not_lag2_f_difference  < istar) '] ; % effective lower bound on the nominal interest rates
constraint_relax2 = [ '(r_not_f_difference > istar) & (r_not_lag1_f_difference  > istar)',...
                      '& (r_not_lag2_f_difference  > istar) '];
...
% [loss_FG_2, loss_FG_2_h, loss_FG_2_f ] = func_cal_walfare(P,pi_h_piecewise,pi_f_piecewise,x_h_piecewise,x_f_piecewise,r_h_piecewise-istar,r_f_piecewise-istar );
 [loss_FG_2, loss_FG_2_h, loss_FG_2_f ] = func_cal_walfare(P,pi_h_piecewise,pi_f_piecewise,x_h_piecewise,x_f_piecewise,r_h_piecewise,r_f_piecewise );

save('./cal_results/loss_FG_2.mat', 'loss_FG_2', 'loss_FG_2_h','loss_FG_2_f');
% load('loss_zlb.mat');
% 
% 
% ti = linspace(0,12,13);
...
%%
 save ./cal_results/IRF_FG_2 line1_fg;
 ...
 figtitle = '';
 figlabel= '';
 legendlist = cellstr(char('FG  2Q Extra','ZLB'));
